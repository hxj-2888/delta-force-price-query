...
// Cloudflare Worker 后端地址（需与 index.html 中 window.__WORKER_BASE 保持一致）
const WORKER_BASE = 'https://delta-force-api.hexiangjie694.workers.dev';
...
async function recordPrices() {
  try {
    ...
    const res = await fetch(WORKER_BASE + '/api/proxy', {
      method: 'POST',
