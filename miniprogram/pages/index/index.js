...
Page({
  ...
  updateCategoryIcons(allItems) {
    ...
    if (logisticsItem) {
      picks['all'] = logisticsItem.pic;
    } else if (!picks['all']) {
      // 兜底：使用任意有图片的物品作为"全部物品"图标
      const fallbackItem = allItems.find(i => i.pic && i._category && i._category !== 'key');
      if (fallbackItem) picks['all'] = fallbackItem.pic;
    }
