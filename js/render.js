...
function renderList(items, showCategory) {
  ...
  document.getElementById('listStats').innerHTML = '\n      <span>共 ' + sorted.length + ' 件 \xB7 第 ' + currentPage + '/' + totalPages + ' 页</span>\n      ' + (sorted.length > 0 ? '<span>更新于 ' + formatTime(sorted[0].is_get_time) + '</span>' : '') + '\n    ';
  const content = document.getElementById('listContent');
  if (sorted.length === 0) {
    content.innerHTML = '<div class="empty-container"><div class="empty-icon">-</div><div class="empty-text">暂无物品数据</div></div>';
  ...
  content.innerHTML = pageItems.map(function(item) {
    ...
    var picHtml = item.pic
      ? '<img src="' + sanitizeUrl(item.pic) + '" alt="" loading="lazy" onerror="this.parentElement.innerHTML=\'<span class=pic-placeholder>-</span>\'">'
...
function renderRecentViews() {
  ...
  container.innerHTML = views.map(function(item) {
    const bl = item.bl || 0;
    var picHtml = item.pic
      ? '<img src="' + sanitizeUrl(item.pic) + '" alt="" loading="lazy" onerror="this.parentElement.innerHTML=\'<span class=pic-placeholder style=font-size:20px>-</span>\'">'
...
function renderDetail(item) {
  ...
  var picHtml = item.pic
    ? '<img src="' + sanitizeUrl(item.pic) + '" alt="" onerror="this.parentElement.innerHTML=\'<span style=font-size:36px>-</span>\'">'
  ...
  // 异步拉取 Cloudflare Worker 云端历史数据，丰富图表
  setTimeout(function() {
...
function renderPriceMovers() {
  ...
  content.innerHTML = controlsHtml +
    ...
    items.map(function(item) {
      ...
      var gradeBg = (item._category !== 'gun' && item.grade) ? 'background:' + getGradeColor(item.grade) + '15;border-color:' + getGradeColor(item.grade) + '30;' : '';
      var gradeDiamond = (item._category !== 'gun' && item.grade) ? '<div class="grade-diamond" style="background:' + getGradeColor(item.grade) + '"></div>' : '';
      var picHtml = item.pic ? '<img src="' + sanitizeUrl(item.pic) + '" alt="" loading="lazy" onerror="this.parentElement.innerHTML=\'<span class=pic-placeholder>-</span>\'">' : '<span class="pic-placeholder">-</span>';
...
function generatePriceCurveSVG(pricePoints) {
  ...
  var paddedMax = maxPrice + priceRange * 0.08;
  ...
  dataPoints.forEach(function(dp) {
    var key = Math.round(dp.price / (priceRange > 100 ? 10 : 1)) * (priceRange > 100 ? 10 : 1);
    if (!seenY[key]) { seenY[key] = true; yDataLabels.push(dp); }
  });
  if (yDataLabels.length > 5) {
    yDataLabels = [
      yDataLabels[0],
      yDataLabels[Math.floor(yDataLabels.length / 3)],
      yDataLabels[Math.floor(yDataLabels.length * 2 / 3)],
      yDataLabels[yDataLabels.length - 1]
    ];
  }

  var xDays = {};
  dataPoints.forEach(function(dp) { xDays[dp.day] = true; });
  [30, 25, 20, 15, 10, 5, 0].forEach(function(d) { xDays[d] = true; });
  var xTicks = Object.keys(xDays).map(Number).sort(function(a, b) { return b - a; });
  var xTickLabels = {};
  dataPoints.forEach(function(dp) {
    if (dp.day === 0) xTickLabels[0] = '当前';
    else if (dp.day === 30) xTickLabels[30] = '30天前';
    else if (dp.day % 5 === 0) xTickLabels[dp.day] = dp.day + '天前';
  });

  var rawMarkers = dataPoints.map(function(dp) {
    return { x: dp.x, y: dp.y, price: dp.price, day: dp.day, isCurrent: dp.day === 0, isHist: dp.hist, isCloud: dp.cloud };
  });
  var seenDay = {};
  rawMarkers = rawMarkers.filter(function(m) {
    if (seenDay[m.day]) return false;
    seenDay[m.day] = true;
    return true;
  });
  rawMarkers.sort(function(a, b) { return a.x - b.x; });
  var markers = [];
  var lastLabelX = -100;
  rawMarkers.forEach(function(m) {
    var loff = 0;
    if (m.x - lastLabelX < 32) { loff = (markers.length % 2 === 0) ? -16 : 8; }
    lastLabelX = m.x;
    markers.push({ x: m.x, y: m.y, price: m.price, day: m.day, isCurrent: m.isCurrent, isHist: m.isHist, isCloud: m.isCloud, loff: loff });
  });
  ...
  return '<svg viewBox="0 0 ' + W + ' ' + H + '" xmlns="http://www.w3.org/2000/svg">' +
    '<defs>' +
      ...
      '</linearGradient>' +
    '</defs>' +
    yDataLabels.map(function(yl) {
      return '<line x1="' + PAD.left + '" y1="' + yl.y.toFixed(1) + '" x2="' + (W - PAD.right) + '" y2="' + yl.y.toFixed(1) + '" stroke="rgba(255,255,255,0.08)" stroke-dasharray="3,4"/>' +
        '<text x="' + (PAD.left - 6) + '" y="' + yl.y.toFixed(1) + '" text-anchor="end" fill="#888" font-size="9" dominant-baseline="middle">\xA5' + shortPrice(yl.price) + '</text>';
    }).join('') +
    ...
    markers.map(function(m) {
      ...
      var dot = '<circle cx="' + m.x.toFixed(1) + '" cy="' + m.y.toFixed(1) + '" r="' + r + '" fill="' + dotColor + '" stroke="' + strokeColor + '" stroke-width="1"/>';
      var ly = m.y - 9 + (m.loff || 0);
      dot += '<text x="' + m.x.toFixed(1) + '" y="' + ly.toFixed(1) + '" text-anchor="middle" fill="' + (m.isCurrent ? '#ffd700' : '#bbb') + '" font-size="9" font-weight="' + (m.isCurrent ? 'bold' : 'normal') + '">\xA5' + shortPrice(m.price) + '</text>';
      return dot;
    }).join('') +
  '</svg>';
}
